...
let isOverdue (currentDate: DateTime) (copy: BookCopy) : bool =
    match copy.Status with
    | Borrowed (_, dueDate) -> currentDate > dueDate
    | _ -> false

let calculatePenalty (currentDate: DateTime) (dailyRate: decimal) (copy: BookCopy) : decimal =
    match copy.Status with
    | Borrowed (_, dueDate) when currentDate > dueDate ->
        let daysOverdue = (currentDate - dueDate).Days
        decimal daysOverdue * dailyRate
    | _ -> 0m

let isCurrentlyBorrowedBy (readerId: ReaderId) (copy: BookCopy) : bool =
    match copy.Status with
    | Borrowed (r, _) -> r.Id = readerId
    | _ -> false
